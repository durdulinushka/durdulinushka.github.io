...
const OverdueTasksStats = () => {
  ...
  const fetchOverdueData = async () => {
    ...
    try {
      ...
      const transformedTasks: OverdueTask[] = (tasksData || []).map(task => {
        ...
        return {
          ...
          assignee_name: task.profiles?.full_name || 'Не назначен',
      ...
      // Получаем статистику
      const { data: statsData, error: statsError } = await supabase.rpc('get_overdue_stats');
      if (statsError) throw statsError;

      if (statsData && statsData.length > 0) {
        setOverdueStats({
          total_overdue: Number(statsData[0].total_overdue) || 0,
          overdue_urgent: Number(statsData[0].overdue_urgent) || 0,
          overdue_long_term: Number(statsData[0].overdue_long_term) || 0,
          days_overdue_avg: Number(statsData[0].days_overdue_avg) || 0
        });
      }
    } catch (error) {
  ...
  if (overdueStats.total_overdue === 0) {
    return (
      <Card>
        <CardContent className="p-6 text-center">
          ...
          <p className="text-muted-foreground">Все задачи выполнены в срок!</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-red-200 bg-red-50">
        <CardHeader>
          <CardTitle className="text-red-800 flex items-center gap-2">
            ...
            Просроченные задачи - Сводка
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="text-center">
              <div className="text-3xl font-bold text-red-600">{overdueStats.total_overdue}</div>
              <div className="text-sm text-red-700">Всего просрочено</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-red-600">{overdueStats.overdue_urgent}</div>
              <div className="text-sm text-red-700">Срочных</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-red-600">{overdueStats.overdue_long_term}</div>
              <div className="text-sm text-red-700">Долгосрочных</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-red-600">{Math.round(overdueStats.days_overdue_avg)}</div>
              <div className="text-sm text-red-700">Средн. дней просрочки</div>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Детальный список просроченных задач</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
...
export default OverdueTasksStats;